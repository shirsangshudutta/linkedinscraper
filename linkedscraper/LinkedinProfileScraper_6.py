...
#%%
query="select Name,company from  linkedin.pagesvisited"
...
#%% Research profiles
query="select Name,designation from  linkedin.pagesvisited"
df = pd.read_sql(query,connection)
...
# for container in x.count:
#     x.bar_label(count)
#%%
labels=[ count for count in x]
labels
#%%ending here

import matplotlib as plt
query="select Name,current_location from  linkedin.pagesvisited"
df = pd.read_sql(query,connection)
x = df.groupby('current_location').size().reset_index(name='count').sort_values(by=['count'],ascending=False)[:10]
x.plot.bar(x='current_location',y='count')
#%%
...
#     data = (datetime.now(),len(visitedProfiles),len(frontier),total_time,changed)
#     print(data)
#     cursor.execute(sql1,data)
#     connection.commit() 
#     print(cursor.rowcount, "record(s) onserted")            
