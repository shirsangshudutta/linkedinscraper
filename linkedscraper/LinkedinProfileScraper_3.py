            ...
            for b in profile.find_all("span", {"class": "entity-result__title-text t-16"}):
                ...
                for f in b.find_all("a",{"class":"app-aware-link"}) :
                    ...
                    # profileIds=getNewProfileIDs(BeautifulSoup(browser.page_source), profilesQueued)
                        ...
                        if (html_source.find("div",{"class":"entity-result__primary-subtitle t-14 t-black"}).find(text=re.compile("Jadavpur University",re.IGNORECASE))):
...
# df = pd.DataFrame(z)
# df
# if  thisdict is not None:
#     for x, y in thisdict.items():
#         print(x,y)  
#%%    
link='https://www.linkedin.com/in/arkadeep-de-121a35196/?miniProfileUrn=urn%3Ali%3Afs_miniProfile%3AACoAAC4SLWkBDGwEUKSWly2otV9S4Gbws4kYY7c'
Recdict=[]
browser.get(link)
# download_urls(link)
time.sleep(random.randint(1,5))
...
x = df.groupby('Current_location').size().to_frame()
type(x)
ax = x.plot(kind='barh', figsize=(8, 10), color='#86bf91', zorder=2, width=0.85)
...
# Set y-axis label
ax.set_ylabel("Current_location", labelpad=20, weight='bold', size=12)
#%%
